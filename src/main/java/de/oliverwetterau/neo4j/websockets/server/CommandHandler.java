...
public class CommandHandler {
    ...
    public void handleMessage(final WebSocketSession session, final JsonNode jsonMessage) throws Exception {
        ...
        try {
            if (jsonMessage.has(CommandParameters.LANGUAGE) && jsonMessage.has(CommandParameters.COUNTRY)) {
                locale = new Locale(jsonMessage.get(CommandParameters.LANGUAGE).asText(), jsonMessage.get(CommandParameters.COUNTRY).asText());
            } else {
                locale = Locale.US;
            }
